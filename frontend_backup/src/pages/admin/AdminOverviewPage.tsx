...
export default function AdminOverviewPage() {
  ...
  // Calculate pending approvals - use totals from overview data if available, otherwise count from users array
  ...
  // Don't block rendering on errors - show partial data instead
  // Individual API calls are handled gracefully in useAdminOverview
  if (error && !data) {
      ...
      <div className="space-y-6">
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard
            ...
            value={totals?.teachers ?? teacherStats?.total ?? 0}
            icon={<GraduationCap className="h-5 w-5" />}
            description={`${teacherStats?.active || 0} active`}
          />
          <StatCard
            title="Total Students"
            value={totals?.students ?? studentStats?.total ?? 0}
            icon={<Users className="h-5 w-5" />}
            description={`${studentStats?.active || 0} active`}
          />
          <StatCard
            title="Total Classes"
            value={classes?.length ?? classStats?.total ?? 0}
