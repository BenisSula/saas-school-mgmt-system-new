/**
 * Load Test for Export Endpoints
 * Tests export functionality under load
 * 
 * Usage: node scripts/load-tests/export-load-test.js
 */
 ...
 * Make export request
 */
async function makeExportRequest(token, format = 'csv') {
...
async function runLoadTest() {
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    for (let i = 0; i < batchSize; i++) {
      batchPromises.push(makeExportRequest(authToken, 'csv'));
    }
