 ...
 * Tests file upload functionality under load
 * 
 * Usage: node scripts/load-tests/upload-load-test.js
 */
...
function createTestFile() {
  const content = 'This is a test file for load testing. '.repeat(30); // ~1KB
 ...
 * Make upload request
 */
async function makeUploadRequest(token) {
...
async function runLoadTest() {
  console.log('\n🚀 Starting File Upload Load Test');
  ...
  for (let batch = 0; batch < batches; batch++) {
    ...
    for (let i = 0; i < batchSize; i++) {
      batchPromises.push(makeUploadRequest(authToken));
    }
