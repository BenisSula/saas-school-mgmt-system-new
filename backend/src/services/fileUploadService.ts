  ...
  if (USE_S3 && S3_BUCKET && S3_ACCESS_KEY_ID && S3_SECRET_ACCESS_KEY) {
    ...
    try {
      ...
      try {
        ...
        console.warn('[fileUploadService] AWS SDK not installed, falling back to local storage. Install @aws-sdk/client-s3 for S3 support.');
        throw new Error('S3 SDK not available');
      }
      
      if (!s3Module || !s3Module.S3Client || !s3Module.PutObjectCommand) {
        console.warn('[fileUploadService] AWS SDK not properly installed, falling back to local storage.');
        throw new Error('S3 SDK not available');
      }
...
export async function deleteFileUpload(
  ...
  return withDbClient(async (client) => {
    ...
    try {
      if (USE_S3 && S3_BUCKET && S3_ACCESS_KEY_ID && S3_SECRET_ACCESS_KEY && file.file_url.startsWith('https://')) {
        // S3 deletion
        let s3Module: any;
        try {
          // eslint-disable-next-line @typescript-eslint/no-implied-eval
