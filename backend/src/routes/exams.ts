...
import { createExam, createExamSession, listExams, getGradeScales, deleteExam } from '../services/examService';
...
router.post('/:examId/sessions', requirePermission('exams:manage'), mutationRateLimiter, async (req, res, next) => {
  const tenant = req.tenant;
  if (!req.tenantClient || !tenant || !req.user) {
    return res.status(500).json({ message: 'Tenant context missing' });
  }
...
router.delete('/:id', requirePermission('exams:manage'), mutationRateLimiter, async (req, res, next) => {
  const tenant = req.tenant;
  if (!req.tenantClient || !tenant || !req.user) {
    return res.status(500).json({ message: 'Tenant context missing' });
  }
