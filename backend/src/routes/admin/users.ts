...
router.put(
  ...
  validateInput(departmentAssignmentSchema, 'body'),
  async (req, res, next) => {
    try {
      ...
      await updateHODDepartment(req.params.id, req.body.department, req.tenant.id, req.user.id);
...
router.delete(
  '/hod/bulk',
  requirePermission('users:manage'),
  validateInput(bulkRemoveHODSchema, 'body'),
  async (req, res, next) => {
    try {
      if (!req.user || !req.tenant) {
        return res.status(500).json(createErrorResponse('User or tenant context missing'));
