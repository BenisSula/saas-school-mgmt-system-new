...
const verifyHOD = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const user = await getUserWithAdditionalRoles(req.tenantClient, req.user.id, req.tenant.id);
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const teachers = await listTeachersUnderHOD(tenantClient, tenant.id, tenant.schema, user.id, {
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      const report = await getDepartmentReport(tenantClient, tenant.id, tenant.schema, user.id, {
