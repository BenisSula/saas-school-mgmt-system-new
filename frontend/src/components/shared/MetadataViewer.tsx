...
export function MetadataViewer({
  ...
  return (
    <div className={`rounded-lg border border-[var(--brand-border)] bg-[var(--brand-surface-secondary)] p-4 ${className}`}>
      ...
      {showRaw ? (
        <div className={`overflow-auto ${maxHeight} bg-[var(--brand-surface)] p-3 rounded border border-[var(--brand-border)]`}>
          ...
          {Object.entries(metadata).map(([key, value]) => (
            <div key={key} className="flex items-start gap-2">
              ...
              <span className="text-xs text-[var(--brand-text-primary)] flex-1">
                {typeof value === 'object' ? (
                  <code className="text-[var(--brand-muted)]">
