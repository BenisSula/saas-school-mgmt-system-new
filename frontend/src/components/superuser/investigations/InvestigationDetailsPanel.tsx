...
export function InvestigationDetailsPanel({ case_, notes = [], evidence = [] }: InvestigationDetailsPanelProps) {
    ...
    () => [
      {
        ...
        fields: [
          ...
          { label: 'Priority', value: case_.priority.toUpperCase(), type: 'badge', badgeClass: priorityColors[case_.priority] },
          { label: 'Status', value: case_.status.charAt(0).toUpperCase() + case_.status.slice(1), type: 'badge', badgeClass: statusColors[case_.status] }
        ]
      },
      {
        title: 'Assignment',
        fields: [
          { label: 'Created By', value: case_.createdBy ? `${case_.createdBy.slice(0, 8)}...` : '—', type: 'text' },
          { label: 'Assigned To', value: case_.assignedTo ? `${case_.assignedTo.slice(0, 8)}...` : 'Unassigned', type: 'text' },
          { label: 'Resolved By', value: case_.resolvedBy ? `${case_.resolvedBy.slice(0, 8)}...` : '—', type: 'text' }
        ]
      },
      {
        title: 'Related Entities',
        fields: [
          { label: 'Related User ID', value: case_.relatedUserId ? `${case_.relatedUserId.slice(0, 8)}...` : '—', type: 'text' },
          { label: 'Related Tenant ID', value: case_.relatedTenantId ? `${case_.relatedTenantId.slice(0, 8)}...` : '—', type: 'text' }
        ]
      },
      {
        title: 'Timeline',
        fields: [
          ...
          { label: 'Investigated At', value: case_.investigatedAt ? formatDateTime(case_.investigatedAt) : '—', type: 'text' },
          { label: 'Resolved At', value: case_.resolvedAt ? formatDateTime(case_.resolvedAt) : '—', type: 'text' },
          { label: 'Closed At', value: case_.closedAt ? formatDateTime(case_.closedAt) : '—', type: 'text' }
        ]
      },
      {
        title: 'Resolution',
    ...
    <div className="space-y-6">
      {fieldGroups.map((group, groupIndex) => (
        <div key={groupIndex} className="card-base p-6">
          <h3 className="mb-4 text-lg font-semibold text-[var(--brand-text-primary)]">{group.title}</h3>
          <dl className="space-y-3">
            {group.fields.map((field, fieldIndex) => (
              <div key={fieldIndex} className="flex flex-col gap-1 sm:flex-row sm:items-start sm:gap-4">
                ...
                <dd className="flex-1 text-sm text-[var(--brand-text-primary)]">
                  {field.type === 'badge' ? (
                    <Badge className={'badgeClass' in field ? (field.badgeClass || '') : ''}>{field.value}</Badge>
