...
export function MetadataCell({ metadata, showReason = true, showNotification = false }: MetadataCellProps) {
    ...
    <div className="flex flex-col gap-1">
      {reason && reason !== '—' && (
        <div className="flex items-center gap-1 text-sm text-[var(--brand-text-secondary)]">
          ...
          <span className="truncate max-w-xs" title={reason}>{reason}</span>
        </div>
      )}
